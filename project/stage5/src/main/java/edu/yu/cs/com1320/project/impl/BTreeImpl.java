...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    public Value get(Key key){
        ...
        if(entry != null){
            if(entry.val != null){ // In mem
                return (Value) entry.val;
            }else{ //On disk
                try{
                    Value valFromPM = persistenceManager.deserialize(key); //Deletes it off disk as well
                    if(valFromPM == null){
                        return null;
                    }else{
                        this.put(key, valFromPM); //Put it back in mem
                        return valFromPM;
                    }
                }catch (IOException e){
                    e.printStackTrace();
    ...
    public Value put(Key key, Value val){
        ...
        if(alreadyThere != null){
            if(alreadyThere.val != null){ //In mem
                ...
                try{
                    ...
                    if(value == null){
                        alreadyThere.val = val; //Not already there
                        return null;
                    }else{
                        alreadyThere.val = val;
                        return value;
    ...
    public void moveToDisk(Key k) throws Exception{
        if(k == null){
            ...
            if(val == null){
                throw new IllegalArgumentException("No Value corresponding to that Key to moveToDisk");
            }else{
